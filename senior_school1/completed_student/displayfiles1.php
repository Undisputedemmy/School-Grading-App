...
if (isset($_GET['admission_no'])) {
    ...
    if ($studentResult) {
        ...
        if (mysqli_num_rows($studentResult) > 0) {
            ...
            $query = "SELECT * FROM z_mathematics1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
            $result = mysqli_query($con, $query);
            
            if (!$result) {
                // Display the specific error message
                die('Error in the query on line 28: ' . mysqli_error($con));
            }
            
            $fresult = mysqli_fetch_assoc($result);

            // Retrieve the results from the eng table
    $engResultQuery = "SELECT * FROM z_english1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $phyResultQuery = "SELECT * FROM z_physics1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $cheResultQuery = "SELECT * FROM z_chemistry1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $ecoResultQuery ="SELECT * FROM z_econs1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $bioResultQuery = "SELECT * FROM z_biology1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $crsResultQuery = "SELECT * FROM z_crs1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $agricResultQuery = "SELECT * FROM z_agric1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $dpResultQuery = "SELECT * FROM z_dp1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $dpResultResult = mysqli_query($con, $dpResultQuery);
    $dpResult = mysqli_fetch_assoc($dpResultResult);

    // Retrieve the results from the che table
    $civicResultQuery = "SELECT * FROM z_civic1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $civicResultResult = mysqli_query($con, $civicResultQuery);
    $civicResult = mysqli_fetch_assoc($civicResultResult);

    // Retrieve the results from the che table
    $accResultQuery = "SELECT * FROM z_accounting1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $accResultResult = mysqli_query($con, $accResultQuery);
    $accResult = mysqli_fetch_assoc($accResultResult);

    // Retrieve the results from the che table
    $yorResultQuery = "SELECT * FROM z_yoruba1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    ...
    $tecResultQuery = "SELECT * FROM z_techdraw1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $tecResultResult = mysqli_query($con, $tecResultQuery);
    $tecResult = mysqli_fetch_assoc($tecResultResult);

    // Retrieve the basic tech from the che table
    $histResultQuery = "SELECT * FROM z_history1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $histResultResult = mysqli_query($con, $histResultQuery);
    $histResult = mysqli_fetch_assoc($histResultResult);

    // Retrieve the basic tech from the che table
    $comResultQuery = "SELECT * FROM z_commerce1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $comResultResult = mysqli_query($con, $comResultQuery);
    $comResult = mysqli_fetch_assoc($comResultResult);

    // Retrieve the basic tech from the che table
    $artResultQuery = "SELECT * FROM z_artcraft1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $artResultResult = mysqli_query($con, $artResultQuery);
    $artResult = mysqli_fetch_assoc($artResultResult);

    // Retrieve the basic tech from the che table
    $catResultQuery = "SELECT * FROM z_catering1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $catResultResult = mysqli_query($con, $catResultQuery);
    $catResult = mysqli_fetch_assoc($catResultResult);

    // Retrieve the basic tech from the che table
    $f_mathResultQuery = "SELECT * FROM z_f_math1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $f_mathResultResult = mysqli_query($con, $f_mathResultQuery);
    $f_mathResult = mysqli_fetch_assoc($f_mathResultResult);

    // Retrieve the basic tech from the che table
    $frenchResultQuery = "SELECT * FROM z_french1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $frenchResultResult = mysqli_query($con, $frenchResultQuery);
    $frenchResult = mysqli_fetch_assoc($frenchResultResult);

    // Retrieve the basic tech from the che table
    $litResultQuery = "SELECT * FROM z_literature1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $litResultResult = mysqli_query($con, $litResultQuery);
    $litResult = mysqli_fetch_assoc($litResultResult);

    // Retrieve the basic tech from the che table
    $geoResultQuery = "SELECT * FROM z_geographyi1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $geoResultResult = mysqli_query($con, $geoResultQuery);
    $geoResult = mysqli_fetch_assoc($geoResultResult);

    // Retrieve the basic tech from the che table
    $govResultQuery = "SELECT * FROM z_government1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $govResultResult = mysqli_query($con, $govResultQuery);
    $govResult = mysqli_fetch_assoc($govResultResult);


    // Retrieve the basic tech from the che table
    $othResultQuery = "SELECT * FROM z_others1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
    $othResultResult = mysqli_query($con, $othResultQuery);
    $othResult = mysqli_fetch_assoc($othResultResult);

    $teaResultQuery = "SELECT * FROM z_t_comment1 WHERE student_id = (SELECT student_id FROM completed_student WHERE admission_no = '$studentId')";
