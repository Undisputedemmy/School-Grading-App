...
if (isset($_POST['submit'])) {
    ...
    $getmidtermScoreScoreQuery = "SELECT COALESCE(total_score, 0) AS midterm_score FROM test_accounting3 WHERE 
  student_id = (SELECT id FROM student_details WHERE admission_no = '$studentId')";
    ...
    // Retrieve the first term score from the accounting1 table
    $getFirstTermScoreQuery = "SELECT COALESCE(terminal_score, 0) AS firstterm_score FROM accounting1 WHERE 
    student_id = (SELECT id FROM student_details WHERE admission_no = '$studentId')";
    ...
    // Retrieve the second term score from the accounting2 table
    $getSecondTermScoreQuery = "SELECT COALESCE(terminal_score, 0) AS secondterm_score FROM accounting2 WHERE 
    student_id = (SELECT id FROM student_details WHERE admission_no = '$studentId')";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $checkQuery = "SELECT * FROM accounting WHERE student_id = '$studentDetailsId'";
        ...
        if ($checkResult && mysqli_num_rows($checkResult) > 0) {
            ...
            $sql = "INSERT INTO accounting (student_id, midterm_score, exam_score, terminal_score, firstterm_score, secondterm_score, cumulative_score, student_grade)
