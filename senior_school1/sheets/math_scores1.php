...
$query = "SELECT m.id, m.student_id, s.name AS student_name, m.midterm_score, m.exam_score,
 m.terminal_score, m.student_grade
          FROM mathematics1 m
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    <head>
        ...
        <title>mathematics Scores</title>
        <style>
            .score-table {
                width: 100%;
    ...
    <body>
        <h2> 1st Term mathematics Scores</h2>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        // Comathematicsert score to "-" if it is 0
    ...
    echo 'No data found in mathematics table.';
}
