...
if (isset($_GET['admission_no'])) {
    ...
    $query = "SELECT mr.*, sd.admission_no, sd.name, sd.class FROM mathematics2 mr, student_details sd WHERE sd.id = mr.student_id AND sd.admission_no = '$studentId'";
    $result = mysqli_query($con, $query);
    $fresult = mysqli_fetch_assoc($result);

    // Retrieve the results from the eng table
    $engResultQuery = "SELECT er.*, sd.admission_no, sd.name, sd.class FROM english2 er, student_details sd WHERE sd.id = er.student_id AND sd.admission_no = '$studentId'";
    ...
    $phyResultQuery = "SELECT ph.*, sd.admission_no, sd.name, sd.class FROM physics2 ph, student_details sd WHERE sd.id = ph.student_id AND sd.admission_no = '$studentId'";
    ...
    $cheResultQuery = "SELECT ch.*, sd.admission_no, sd.name, sd.class FROM chemistry2 ch, student_details sd WHERE sd.id = ch.student_id AND sd.admission_no = '$studentId'";
    ...
    $ecoResultQuery = "SELECT ec.*, sd.admission_no, sd.name, sd.class FROM econs2 ec, student_details sd WHERE sd.id = ec.student_id AND sd.admission_no = '$studentId'";
    ...
    $bioResultQuery = "SELECT bi.*, sd.admission_no, sd.name, sd.class FROM biology2 bi, student_details sd WHERE sd.id = bi.student_id AND sd.admission_no = '$studentId'";
    ...
    $crsResultQuery = "SELECT cr.*, sd.admission_no, sd.name, sd.class FROM crs2 cr, student_details sd WHERE sd.id = cr.student_id AND sd.admission_no = '$studentId'";
    ...
    $agricResultQuery = "SELECT ag.*, sd.admission_no, sd.name, sd.class FROM agric2 ag, student_details sd WHERE sd.id = ag.student_id AND sd.admission_no = '$studentId'";
    ...
    $dpResultQuery = "SELECT co.*, sd.admission_no, sd.name, sd.class FROM dp2 co, student_details sd WHERE sd.id = co.student_id AND sd.admission_no = '$studentId'";
    $dpResultResult = mysqli_query($con, $dpResultQuery);
    $dpResult = mysqli_fetch_assoc($dpResultResult);

    // Retrieve the results from the che table
    $civicResultQuery = "SELECT ci.*, sd.admission_no, sd.name, sd.class FROM civic2 ci, student_details sd WHERE sd.id = ci.student_id AND sd.admission_no = '$studentId'";
    $civicResultResult = mysqli_query($con, $civicResultQuery);
    $civicResult = mysqli_fetch_assoc($civicResultResult);

    // Retrieve the results from the che table
    $accResultQuery = "SELECT bt.*, sd.admission_no, sd.name, sd.class FROM accounting2 bt, student_details sd WHERE sd.id = bt.student_id AND sd.admission_no = '$studentId'";
    $accResultResult = mysqli_query($con, $accResultQuery);
    $accResult = mysqli_fetch_assoc($accResultResult);

    // Retrieve the results from the che table
    $yorResultQuery = "SELECT yo.*, sd.admission_no, sd.name, sd.class FROM yoruba2 yo, student_details sd WHERE sd.id = yo.student_id AND sd.admission_no = '$studentId'";
    ...
    $tecResultQuery = "SELECT te.*, sd.admission_no, sd.name, sd.class FROM techdraw2 te, student_details sd WHERE sd.id = te.student_id AND sd.admission_no = '$studentId'";
    $tecResultResult = mysqli_query($con, $tecResultQuery);
    $tecResult = mysqli_fetch_assoc($tecResultResult);

    // Retrieve the basic tech from the che table
    $histResultQuery = "SELECT hi.*, sd.admission_no, sd.name, sd.class FROM history2 hi, student_details sd WHERE sd.id = hi.student_id AND sd.admission_no = '$studentId'";
    $histResultResult = mysqli_query($con, $histResultQuery);
    $histResult = mysqli_fetch_assoc($histResultResult);

    // Retrieve the basic tech from the che table
    $comResultQuery = "SELECT se.*, sd.admission_no, sd.name, sd.class FROM commerce2 se, student_details sd WHERE sd.id = se.student_id AND sd.admission_no = '$studentId'";
    $comResultResult = mysqli_query($con, $comResultQuery);
    $comResult = mysqli_fetch_assoc($comResultResult);

    // Retrieve the basic tech from the che table
    $artResultQuery = "SELECT ac.*, sd.admission_no, sd.name, sd.class FROM artcraft2 ac, student_details sd WHERE sd.id = ac.student_id AND sd.admission_no = '$studentId'";
    $artResultResult = mysqli_query($con, $artResultQuery);
    $artResult = mysqli_fetch_assoc($artResultResult);

    // Retrieve the basic tech from the che table
    $catResultQuery = "SELECT cc.*, sd.admission_no, sd.name, sd.class FROM catering2 cc, student_details sd WHERE sd.id = cc.student_id AND sd.admission_no = '$studentId'";
    $catResultResult = mysqli_query($con, $catResultQuery);
    $catResult = mysqli_fetch_assoc($catResultResult);

    // Retrieve the basic tech from the che table
    $f_mathResultQuery = "SELECT mu.*, sd.admission_no, sd.name, sd.class FROM f_math2 mu, student_details sd WHERE sd.id = mu.student_id AND sd.admission_no = '$studentId'";
    $f_mathResultResult = mysqli_query($con, $f_mathResultQuery);
    $f_mathResult = mysqli_fetch_assoc($f_mathResultResult);

    // Retrieve the basic tech from the che table
    $frenchResultQuery = "SELECT fr.*, sd.admission_no, sd.name, sd.class FROM french2 fr, student_details sd WHERE sd.id = fr.student_id AND sd.admission_no = '$studentId'";
    $frenchResultResult = mysqli_query($con, $frenchResultQuery);
    $frenchResult = mysqli_fetch_assoc($frenchResultResult);

    // Retrieve the basic tech from the che table
    $litResultQuery = "SELECT li.*, sd.admission_no, sd.name, sd.class FROM literature2 li, student_details sd WHERE sd.id = li.student_id AND sd.admission_no = '$studentId'";
    $litResultResult = mysqli_query($con, $litResultQuery);
    $litResult = mysqli_fetch_assoc($litResultResult);

    // Retrieve the basic tech from the che table
    $geoResultQuery = "SELECT gr.*, sd.admission_no, sd.name, sd.class FROM geographyi2 gr, student_details sd WHERE sd.id = gr.student_id AND sd.admission_no = '$studentId'";
    $geoResultResult = mysqli_query($con, $geoResultQuery);
    $geoResult = mysqli_fetch_assoc($geoResultResult);

    // Retrieve the basic tech from the che table
    $govResultQuery = "SELECT gv.*, sd.admission_no, sd.name, sd.class FROM government2 gv, student_details sd WHERE sd.id = gv.student_id AND sd.admission_no = '$studentId'";
    $govResultResult = mysqli_query($con, $govResultQuery);
    $govResult = mysqli_fetch_assoc($govResultResult);


    // Retrieve the basic tech from the che table
    $othResultQuery = "SELECT er.*, sd.admission_no, sd.name, sd.class FROM others2 er, student_details sd WHERE sd.id = er.student_id AND sd.admission_no = '$studentId'";
    $othResultResult = mysqli_query($con, $othResultQuery);
    $othResult = mysqli_fetch_assoc($othResultResult);

    $teaResultQuery = "SELECT er.*, sd.admission_no, sd.name, sd.class FROM t_comment2 er, student_details 
