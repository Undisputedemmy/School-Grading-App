...
if (isset($_GET['admission_no'])) {
    ...
    $pvsResultQuery = "SELECT ph.*, sd.admission_no, sd.name, sd.class FROM pvs1 ph, student_details sd WHERE sd.id = ph.student_id AND sd.admission_no = '$studentId'";
    ...
    $nvResultQuery = "SELECT ch.*, sd.admission_no, sd.name, sd.class FROM nv1 ch, student_details sd WHERE sd.id = ch.student_id AND sd.admission_no = '$studentId'";
    ...
    $bstResultQuery = "SELECT ec.*, sd.admission_no, sd.name, sd.class FROM bst1 ec, student_details sd WHERE sd.id = ec.student_id AND sd.admission_no = '$studentId'";
    ...
    $busResultQuery = "SELECT co.*, sd.admission_no, sd.name, sd.class FROM business1 co, student_details sd WHERE sd.id = co.student_id AND sd.admission_no = '$studentId'";
    ...
    // Retrieve the basic tech from the nv table
    $ccaResultQuery = "SELECT cc.*, sd.admission_no, sd.name, sd.class FROM cca1 cc, student_details sd WHERE sd.id = cc.student_id AND sd.admission_no = '$studentId'";
...
if (isset($pvsResult['terminal_score'])) {
    $totalScore += $pvsResult['terminal_score'];
    $numberOfSubjectsTaken++;
}

if (isset($nvResult['terminal_score'])) {
    $totalScore += $nvResult['terminal_score'];
    $numberOfSubjectsTaken++;
}

if (isset($bstResult['terminal_score'])) {
    $totalScore += $bstResult['terminal_score'];
    $numberOfSubjectsTaken++;
}
...
if (isset($busResult['terminal_score'])) {
    $totalScore += $busResult['terminal_score'];
    $numberOfSubjectsTaken++;
}
...
if (isset($ccaResult['terminal_score'])) {
    $totalScore += $ccaResult['terminal_score'];
    $numberOfSubjectsTaken++;
}
