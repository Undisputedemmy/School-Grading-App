...
if (isset($_POST['submit'])) {
  ...
  $getmidtermScoreScoreQuery = "SELECT COALESCE(total_score, 0) AS midterm_score FROM test_business3 WHERE 
  ...
  // Retrieve the first term score from the business1 table
  $getFirstTermScoreQuery = "SELECT COALESCE(terminal_score, 0) AS firstterm_score FROM business1 WHERE 
  student_id = (SELECT id FROM student_details WHERE admission_no = '$studentId')";
  ...
  // Retrieve the second term score from the business2 table
  $getSecondTermScoreQuery = "SELECT COALESCE(terminal_score, 0) AS secondterm_score FROM business2 WHERE 
  student_id = (SELECT id FROM student_details WHERE admission_no = '$studentId')";
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    // Check if the scores already exist in the business table for the given student
    $checkQuery = "SELECT * FROM business WHERE student_id = '$studentDetailsId'";
    ...
    if ($checkResult && mysqli_num_rows($checkResult) > 0) {
      ...
      // Insert the result into the business table with the associated student ID
      $sql = "INSERT INTO business (student_id, midterm_score, exam_score, terminal_score, firstterm_score, secondterm_score, cumulative_score, student_grade)
      ...
      if ($res) {
        ...
        $message = "business scores submitted successfully!";
