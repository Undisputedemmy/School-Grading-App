...
include 'connect5.php'; // Connection for project_school database
include 'connect6.php'; // Connection for project_school6 database
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['promote'])) {
    ...
    $admissionNo = mysqli_real_escape_string($con_project_school6, $_POST['admission_no']);

    $fetchStudentQuery = "SELECT * FROM student_details WHERE admission_no = '$admissionNo'";
    $fetchStudentResult = mysqli_query($con_project_school6, $fetchStudentQuery);
    ...
    if ($fetchStudentResult && $row = mysqli_fetch_assoc($fetchStudentResult)) {
        ...
        $insertStudentQuery = "INSERT INTO project_school6.student_details (name, admission_no, class, department, A_session)
                               VALUES ('{$row['name']}', '$admissionNo', 'SSS 3', 'department', '$newAcademicSession')";

        $insertStudentResult = mysqli_query($con_project_school6, $insertStudentQuery);

        if ($insertStudentResult) {
            // Step 3: Update Student's Class in Class 1
            $updateClassQuery = "UPDATE project_school6.student_details
                                SET class = 'SSS 2'
                                WHERE admission_no = '$admissionNo'";

            $updateClassResult = mysqli_query($con_project_school6, $updateClassQuery);
            ...
            if ($updateClassResult) {
                ...
                echo "Error updating student's class in Class 2: " . mysqli_error($con_project_school6);
            }
        } else {
            echo "Error inserting student into Class 3: " . mysqli_error($con_project_school6);
        }
    } else {
        echo "Error fetching student details from Class 2: " . mysqli_error($con_project_school6);
    }
}
?>
